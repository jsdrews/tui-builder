...
//
// `o` is here because cmd/tui-builder binds it to the output console. A
// key the shell takes is a key no config may bind.
//
...
//   - t: tui-builder never sets app.Options.ThemeKey, so nothing
//     consumes it. (README claims `t` cycles themes; it does not.)
...
func validateActionBindings(bindings []ActionBinding, refs map[string]int, components map[string]*Component, actions map[string]*Action, path string) error {
	seen := map[string]int{}
